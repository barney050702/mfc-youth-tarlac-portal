...
- **Authorized Security Passkey:** `admin123` _(also accepts `mfc2026` or `barney2026`)_
...
- **Forgot Firebase Password Reset Link:** Leaders can click _Forgot Firebase Password?_ on the login modal to receive an automated Firebase password reset link (`sendPasswordResetEmail`).
- **Remember Executive Email:** Leaders can toggle _Remember Executive Email_ to auto-load their email on future sessions.
